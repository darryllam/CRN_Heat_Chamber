...
from tensorflow.python.training.tracking.util import streaming_restore #time for sleeps
...
# target test data
val_data, _test_file_names = get_data(train_cols,parsed_args.part_col, parsed_args.val_path, data_len, True)
val_scaled = val_data
...
if(only_predict_flag == 0):
    ...
    train_source_data, _source_file_names = get_data(train_cols,parsed_args.part_col, parsed_args.source_path, data_len, True)
    print("loading target training data")
    train_target_data, _target_file_names = get_data(train_cols,parsed_args.part_col, parsed_args.test_path, data_len, True)
    raw_data = np.concatenate((train_source_data, train_target_data), axis=0)
    train_target_start = len(train_source_data)
    scaled = raw_data
...
if(len_scaled_run_cols_arg != None):
    for i in range(val_scaled.shape[2]- len(scaled_run_cols_arg), val_scaled.shape[2]):
        if(only_predict_flag == 0): 
            ...
            val_scaled[:,:,i] = col_scalers[i].transform(val_data[:,:,i])
    if(only_predict_flag == 0):
        col_scalers[len(col_scalers)] = train_target_start
        joblib.dump(col_scalers[i], out_file_name[:-3] + str(i) + ".pkl") 
    else:
        col_scalers[len(col_scalers)] = joblib.load(in_file_name[:-3] + str(i) + ".pkl")
        train_target_start = col_scalers[len(col_scalers)] 


for t in range(0,val_scaled.shape[0]):
    val_scaled[t,:,:] = delay_series(val_scaled[t,:,1:],val_scaled[t,:,0],0)

val_scaled_tl, test_truth = feda.source_transform(val_scaled[0], val_scaled.shape[2] - 1, False)
val_scaled_tl = val_scaled_tl[np.newaxis, :]

val_scaled_reshape = np.zeros((val_scaled_tl.shape[0], data_len, windows, val_scaled_tl.shape[2]))


for t in range(0,val_scaled_tl.shape[0]):
    for i in range(0,windows):
        data = val_scaled_tl[t,:,:]
        for td in range(0,i):
...
if(only_predict_flag == 0):
    ...
    scaled_tl = np.zeros((scaled.shape[0], scaled.shape[1], val_scaled_tl.shape[2]))
    train_truth = np.zeros((scaled.shape[0], scaled.shape[1]))

    for t in range(0,scaled.shape[0]):
        scaled[t,:,:] =  delay_series(scaled[t,:,1:],scaled[t,:,0],0)
        if t < train_target_start:
            scaled_tl[t], train_truth[t] = feda.source_transform(scaled[t], scaled.shape[2] - 1, False)
        else:
            scaled_tl[t], train_truth[t] = feda.target_transform(scaled[t], scaled.shape[2] - 1, False)
    ...
    scaled_reshape = np.zeros((scaled_tl.shape[0], data_len, windows, scaled_tl.shape[2]))
    for t in range(0,scaled_tl.shape[0]):
        if t < train_target_start:
            scaled_tl[t], train_truth[t] = feda.source_transform(scaled[t], scaled.shape[2] - 1, False)
        else:
            scaled_tl[t], train_truth[t] = feda.target_transform(scaled[t], scaled.shape[2] - 1, False)
