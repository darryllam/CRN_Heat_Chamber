...
for i in range(0,val_scaled_reshape.shape[0]):
    ...
    for j in range(0, end_time+1,live_predict_step):
        ...
        pyplot.plot(val_scaled_copy[i,:,0,0], label='Time') #Inner Temp
        pyplot.plot(val_scaled_copy[i,:,0,2], label='Part Temp') #Inner Temp
        pyplot.plot(val_scaled_copy[i,:,0,1], label='Air Temperature') #Inner Temp
        pyplot.plot(yhat,  label='Part Temperature Prediction')
